...
# ==========================================
print("--- 🛠️ 環境初始化 ---")
API_KEY = getpass.getpass("請輸入您的 GEMINI_API_KEY: ")
...
# 🧠 核心提煉邏輯 (全方位整合版)
# ==========================================
def distill_file(client, file_path, model_choice):
    ...
    gemini_file = None  # 確保 finally 中可安全存取
    try:
        # 1. 上傳檔案至 Gemini 雲端
        ...
        if match:
            ...
            pro_naming = re.sub(r'[\\/*?:"<>|]', "", pro_naming)  # 清理檔名非法字元

            final_md_name = f"{pro_naming}_summary.md"
            final_md_path = OUTPUT_DIR / final_md_name
...
def main():
    ...
    for i, pdf in enumerate(pdf_files):
        ...
        # 🔄 重複檢查：掃描 OUTPUT_DIR 中是否已存在以該 stem 開頭的摘要
